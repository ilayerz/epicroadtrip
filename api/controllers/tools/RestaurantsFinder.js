...
class RestaurantsFinder {
    ...
    async parse(body) {
        ...
        for (let i = 0; i < elements.length; i++) {
            element = elements[i]
            let rate =  await RestaurantsFinder.changeRating(element.properties.rate)
            const restaurant = {
                position: {
    ...
    search(lat, lng, radius) {
        ...
        var req = unirest("GET", `https://api.opentripmap.com/0.1/en/places/radius?radius=${radius}&lon=${lng}&lat=${lat}&src_geom=wikidata&src_attr=wikidata&kinds=restaurants&apikey=${API_KEY}`);
...
module.exports = RestaurantsFinder;
...
class TransportFinder {
    ...
    async parse(body) {
        ...
        for (let i = 0; i < elements.length; i++) {
            element = elements[i]
            let rate =  await TransportFinder.changeRating(element.properties.rate)
            const transport = {
                position: {
    ...
    search(lat, lng, radius) {
        ...
        var req = unirest("GET", `https://api.opentripmap.com/0.1/en/places/radius?radius=${radius}&lon=${lng}&lat=${lat}&kinds=transport&apikey=${API_KEY}`);
...
module.exports = TransportFinder;
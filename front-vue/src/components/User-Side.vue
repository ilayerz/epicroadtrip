<template>
  <div class="user-side">
    <div :class="!auth.isUserAuthenticated() ? 'user-action' : 'connected-user-action'">
      <!--  Title  -->
      <div class="title">
      ...
      <!--  Tuto  -->
      <div v-if="planning" class="tutorial">
        <div v-on:click="$emit('tutorial')" class="btn-start">
          <h2>Tutorial</h2>
        </div>
      </div>
      ...
      <!--  Planning Zone  -->
      <div class="planning">
        <div v-on:click="planning = true" class="planning-title">
          <h2>Planning Road Trip</h2>
          <BurgerMenu
            :inputDataParent="inputData"
            :originMap="originMap"
            :destinationMap="destinationMap"
            :addHistory="addHistory"
            v-on:modalMap="$emit('modalMap', $event)"
            v-on:removeHistory="$emit('removeHistory', $event)"
          ></BurgerMenu>
        </div>
        <div v-if="planning" class="planning-block">
          <div class="line"></div>
          <div class="block-container">
            <div class="block">
              <h3>Dates planner:</h3>
              <div class="date">
                <input id="dateStart" name="dateStart" v-model="inputData.dateStart" type="date"/>
              </div>
              <div class="switch">
                <img id="reverseDate" :src="Switch" alt="switch" v-on:click="reverseData('date')">
              </div>
              <div class="date">
                <input id="dateEnd" name="dateEnd" v-model="inputData.dateEnd" type="date"/>
              </div>
            </div>
      ...
      <div id="addHistoryDesktop" v-if="addHistory !== null && planning">
    ...
    <User
      v-if="saveData === null"
      v-on:closePlanning="planning = $event"
      v-on:allRoute="$emit('allRoute')"
    >
    </User>
    <div v-else v-on:click="returnOrigin" class="return">
      <h3>Return to trip creation !</h3>
    </div>
  </div>
</template>
...
import { postRoute } from '@/services/api/user';
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const checkAuth = ref(auth.isUserAuthenticated());
    ...
    const saveRoute = () => {
      const data = {
        origin: props.originMap ? props.originMap.origin : null,
        destination: props.destinationMap ? props.destinationMap.destination : null,
        date: {
          start: inputData.value.dateStart,
          end: inputData.value.dateEnd,
        },
        history: props.addHistory ? props.addHistory : null,
      };
      postRoute(JSON.stringify(data), auth.token.value).then((response) => {
    ...
    watch(
      () => auth,
      () => {
        checkAuth.value = auth.isUserAuthenticated();
      },
      { deep: true },
    );
    ...
    const returnOrigin = () => {
      ...
      window.location.href = window.location.href.split('/route/')[0];
    };

    onMounted(() => {
      if (props.saveData !== null) {
        inputData.value.dateStart = props.saveData.date.start;
        inputData.value.dateEnd = props.saveData.date.end;
      }
    });
